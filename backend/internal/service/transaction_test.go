...
func TestScenarioCreatorCannotVerifyOwnSimulation(t *testing.T) {
	...
	service := NewRolloverScenarioService(repository.NewRolloverScenarioRepository(db), nil, nil, nil, repository.NewRolloverSignoffRepository(db), repository.NewAuditRepository(db), repository.NewTransactionManager(db))
...
func newScenarioTestDB(t *testing.T) *gorm.DB {
	...
	if err := db.AutoMigrate(&model.TrustAnchor{}, &model.RolloverScenario{}, &model.RolloverSignoff{}, &model.AuditLog{}); err != nil {
		t.Fatal(err)
	}
	return db
}
...
func TestReplayEvidenceRollsBackWhenAuditAppendFails(t *testing.T) {
	...
	service := NewRolloverScenarioService(repository.NewRolloverScenarioRepository(db), nil, nil, nil, repository.NewRolloverSignoffRepository(db), failingAuditRepository{err: auditErr}, repository.NewTransactionManager(db))
