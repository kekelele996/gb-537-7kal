...
func TestScenarioCreatorCannotVerifyOwnSimulation(t *testing.T) {
	...
	service := NewRolloverScenarioService(repository.NewRolloverScenarioRepository(db), repository.NewImpactSignoffRepository(db), nil, nil, nil, repository.NewAuditRepository(db), repository.NewTransactionManager(db))
...
func newScenarioTestDB(t *testing.T) *gorm.DB {
	...
	if err := db.AutoMigrate(&model.TrustAnchor{}, &model.RolloverScenario{}, &model.ImpactSignoff{}, &model.AuditLog{}); err != nil {
		t.Fatal(err)
	}
	return db
}
...
func TestReplayEvidenceRollsBackWhenAuditAppendFails(t *testing.T) {
	...
	service := NewRolloverScenarioService(repository.NewRolloverScenarioRepository(db), repository.NewImpactSignoffRepository(db), nil, nil, nil, failingAuditRepository{err: auditErr}, repository.NewTransactionManager(db))
